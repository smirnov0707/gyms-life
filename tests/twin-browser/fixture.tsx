...
const known: Record<string, { recoveryPct: number; recoveryBand: TwinRegionState["recoveryBand"] }> = {
...
function snapshot(empty: boolean): TwinSnapshot {
  return {
    calculationVersion: "TEST-FIXTURE-NOT-USER-DATA", computedAt: "2026-09-05T12:00:00Z", evidenceWindowDays: 14, dataAvailable: true,
    regions: KNOWN_MUSCLE_GROUPS.map((region) => {
      const value = empty ? undefined : known[region];
      return { region, provenance: value ? "calculated" : "unknown", recoveryPct: value?.recoveryPct ?? null,
        recoveryBand: value?.recoveryBand ?? "unknown", volumeKg: value ? 1000 : null, lastTrainedHoursAgo: value ? 48 : null };
    }),
  };
}
function Fixture() {
  ...
  return <main style={{ maxWidth: 1000, margin: "auto", padding: 16 }}>
    ...
    <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
      ...
      <button onClick={() => setEmpty((value) => !value)}>{empty ? "Restore evidence" : "Clear evidence"}</button>
    </div>
    {mounted && <TwinSnapshotView data={snapshot(empty)} copy={twinCopyFor("en")} lang="en" label={(region) => region.charAt(0).toUpperCase() + region.slice(1)} />}
  </main>;
}
