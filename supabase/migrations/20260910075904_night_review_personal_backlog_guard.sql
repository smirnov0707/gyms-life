...
begin
 ...
 if p_report->>'status'='completed' and (p_report#>>'{predictions,status}' is distinct from 'completed' or p_report#>>'{hypotheses,status}' is distinct from 'completed' or (p_report ? 'modelLearning' and (p_report#>>'{modelLearning,status}' is distinct from 'completed' or coalesce((p_report#>>'{modelLearning,predictionReview,limited}')::boolean,false) is true)))
   then raise exception 'NIGHT_REVIEW_STAGE_MISMATCH';end if;
