-- Canonical deterministic memory replaces the unused parallel user_insights
-- write path. The legacy table remains retained for historical compatibility,
-- but is no longer exposed to browser clients.
...
-- One active key per user protects both current-life context and derived facts
-- from concurrent server requests creating duplicate active memory entries.
create unique index if not exists user_memory_active_memory_key_unique
  on public.user_memory (user_id, memory_key)
  where memory_key is not null and status = 'active';

-- Reconciles bounded, application-generated facts while retaining historical
-- records. It is private to the trusted app server; users can still correct a
-- resulting memory item, and an explicit user correction is never overwritten.
...
declare
  ...
  v_new_id uuid;
begin
  if jsonb_typeof(p_entries) <> 'array' or jsonb_array_length(p_entries) > 4 then
    raise exception 'Calculated memory entries must be an array of at most four items.'
      using errcode = '22023';
  end if;
  ...
  loop
    ...
    if (v_memory_key = 'derived:training_consistency_28d' and v_memory_type <> 'training_pattern')
      ...
      or (v_memory_key = 'derived:nutrition_logging_14d' and v_memory_type <> 'nutrition_pattern') then
