...
declare
  uid uuid := auth.uid(); revision timestamptz; goal text;
  diet text; allergies text; dislikes text; meals integer; key text;
begin
  ...
  diet:=p_preferences->>'diet'; allergies:=p_preferences->>'allergies'; dislikes:=p_preferences->>'dislikes';
  meals:=(p_preferences->>'mealsPerDay')::integer;
  if diet is null or diet not in ('any','vegetarian','vegan','pescatarian','low carb','gluten free','lactose free')
    or allergies is null or length(allergies)>500 or dislikes is null or length(dislikes)>500
    or meals is null or meals<2 or meals>6
  ...
  if (p_plan->>'kcal_target')::numeric<1000 or (p_plan->>'kcal_target')::numeric>6000
    ...
    or exists(select 1 from jsonb_array_elements(p_plan->'days') day
      ...
      or jsonb_typeof(day->'meals') is distinct from 'array' or jsonb_array_length(day->'meals')<>meals)
  ...
  select p.updated_at,p.goal into revision,goal from public.profiles p where p.id=uid for update;
  ...
  update public.profiles p set diet=commit_generated_meal_plan.diet,
    allergies=commit_generated_meal_plan.allergies,dislikes=commit_generated_meal_plan.dislikes,
    meals_per_day=meals where p.id=uid;
  ...
  values(p_plan_id,uid,p_plan->>'title',goal,diet,allergies,dislikes,
...
grant execute on function public.commit_generated_meal_plan(uuid,timestamptz,jsonb,jsonb,text) to authenticated;
