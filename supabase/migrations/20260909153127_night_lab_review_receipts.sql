-- Per-athlete proof of work. Source facts and model weights remain in their canonical tables.
create table public.night_lab_reviews (
 ...
 snapshot_id uuid references public.athlete_state_snapshots(id) on delete restrict,
 report jsonb not null check(jsonb_typeof(report)='object'),
 created_at timestamptz not null default now(),
 unique(run_id,user_id)
);
create index night_lab_reviews_owner_recent_idx on public.night_lab_reviews(user_id,reviewed_at desc,id desc);
alter table public.night_lab_reviews enable row level security;
revoke all on public.night_lab_reviews from public,anon,authenticated;
grant select on public.night_lab_reviews to authenticated;
grant all on public.night_lab_reviews to service_role;
create policy night_lab_reviews_owner_read on public.night_lab_reviews for select to authenticated using(user_id=(select auth.uid()));

create function public.commit_night_lab_review(p_run_id uuid,p_claimed_at timestamptz,p_user_id uuid,p_report jsonb)
...
begin
 ...
 if p_report->>'status'='completed' and (p_report#>>'{predictions,status}' is distinct from 'completed' or p_report#>>'{hypotheses,status}' is distinct from 'completed')
   then raise exception 'NIGHT_REVIEW_STAGE_MISMATCH';end if;
