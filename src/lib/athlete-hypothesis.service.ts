...
import {
  AthleteDiscoverySchema,
  AthleteHypothesisSchema,
  type AthleteDiscovery,
...
export function buildAthleteHypotheses(state: DigitalAthleteState): AthleteHypothesis[] {
        ...
        evidence: [
          {
            ...
            value: response.ratedSessionsLast28Days,
            unit: "sessions",
            source: "user_reported",
          },
          {
            key: "recent_low_feeling_streak",
            value: response.recentLowFeelingStreak,
            unit: "sessions",
            source: "user_reported",
          },
        ],
  ...
  if (state.behavior.status === "measured") {
    const enoughEvidence =
      state.behavior.usualTrainingDaysLast28Days >= MIN_USUAL_TRAINING_DAYS_FOR_BEHAVIOR_LEARNING;
    const supported =
      enoughEvidence &&
      state.behavior.usualDayCompletionRateLast28Days >= STRONG_USUAL_DAY_COMPLETION_RATE;
    hypotheses.push(
      AthleteHypothesisSchema.parse({
        ...
        evidence: [
          {
            key: "usual_training_days_28d",
            value: state.behavior.usualTrainingDaysLast28Days,
            unit: "days",
            source: "calculated",
          },
          {
            key: "usual_day_completion_rate_28d",
            value: state.behavior.usualDayCompletionRateLast28Days,
            unit: "ratio",
            source: "calculated",
          },
        ],
...
export function buildAthleteDiscoveries(hypotheses: AthleteHypothesis[]): AthleteDiscovery[] {
  return hypotheses.flatMap((hypothesis) =>
    hypothesis.status === "supported" && hypothesis.evidence.length > 0
      ? [
          AthleteDiscoverySchema.parse({
