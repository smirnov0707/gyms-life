...
function estimated1RM(weightKg: number | null, reps: number | null) {
...
function aggregateExercise(logs: SetLog[], slug: string): ExercisePerformance | null {
  ...
  const names = rows.map((row) => row.exercise_name).filter(Boolean);
  const exerciseName = names[0] ?? slug;
  const sessions = new Set(rows.map((row) => row.session_id)).size;
  const totalSets = rows.length;
  const totalReps = rows.reduce((sum, row) => sum + (row.reps ?? 0), 0);
  const totalVolume = Number(rows.reduce((sum, row) => sum + (row.reps ?? 0) * (row.weight_kg ?? 0), 0).toFixed(1));
  ...
  const e1rms = rows.map((row) => estimated1RM(row.weight_kg, row.reps)).filter((value): value is number => value != null);
  ...
  return {
    ...
    latest: { date: latest.created_at, weightKg: latest.weight_kg, reps: latest.reps, rpe: latest.rpe, estimated1RMKg: estimated1RM(latest.weight_kg, latest.reps) },
  };
}

async function loadCompletedData(supabase: any, userId: string) {
  ...
  const { data: logs, error: logError } = await supabase.from("set_logs").select(SET_SELECT).in("session_id", ids).eq("done", true).order("created_at", { ascending: true });
...
export const getPerformanceOverview = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  ...
  return { status: "READY" as const, metrics: { workouts: sessions.length, totalVolume, totalDurationSeconds, totalSets, totalReps, averageRpe: rpes.length ? Number((rpes.reduce((a, b) => a + b, 0) / rpes.length).toFixed(1)) : null }, exercises };
});
...
export const getExerciseProgress = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).validator((input: unknown) => ExerciseInput.parse(input)).handler(async ({ data, context }) => {
  ...
  const points = logs.filter((log) => log.exercise_slug === data.exerciseSlug && log.done && sessionIds.has(log.session_id)).map((log) => ({ date: log.created_at, weightKg: log.weight_kg, reps: log.reps, rpe: log.rpe, estimated1RMKg: estimated1RM(log.weight_kg, log.reps) }));
...
export const getStrengthTrend = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).handler(async ({ context }) => {
  ...
  for (const log of logs) {
    const e1rm = estimated1RM(log.weight_kg, log.reps);
