...
describe("Personalized Twin framing", () => {
  ...
  it("distinguishes cropped, too close and too far observations", () => {
    expect(
      assessTwinFraming({ bodyHeightRatio: 0.75, topMarginRatio: 0.01, bottomMarginRatio: 0.24 }),
    ).toMatchObject({ status: "cropped" });
    expect(
      assessTwinFraming({ bodyHeightRatio: 0.94, topMarginRatio: 0.03, bottomMarginRatio: 0.03 }),
    ).toMatchObject({ status: "too_close" });
    expect(
      assessTwinFraming({ bodyHeightRatio: 0.4, topMarginRatio: 0.3, bottomMarginRatio: 0.3 }),
