...
export function mapTwinSnapshotToVisualState(snapshot: TwinSnapshot): TwinVisualState {
  ...
  const regions = snapshot.regions.map((region) => {
    const known =
      ...
      region.recoveryPct >= 0 &&
      region.recoveryPct <= 100;
    return {
      region: region.region,
      attention: known ? attentionByBand[region.recoveryBand] : "unknown",
      emphasis: known
        ? Number((0.35 + ((100 - region.recoveryPct!) / 100) * 0.65).toFixed(2))
        : 0.18,
    };
  });
  const evidenceCount = regions.filter((region) => region.attention !== "unknown").length;
  const ambientMotion: TwinAmbientMotion =
    !snapshot.dataAvailable || evidenceCount === 0
      ? "still"
      : regions.some((region) => region.attention === "high")
        ? "active"
        : "quiet";
  return TwinVisualStateSchema.parse({
    ...
    evidenceCoverage:
      snapshot.regions.length === 0
        ? 0
        : Number((evidenceCount / snapshot.regions.length).toFixed(3)),
    regions,
  });
}
