...
export const AthleteLearningDomainSchema = z.enum([
  "training_response",
  "training_behavior",
  "recovery",
  "nutrition",
...
export const AthleteHypothesisStatusSchema = z.enum([
...
export const AthleteEvidenceMetricSchema = z
  .object({
...
export const AthleteHypothesisSchema = z
  .object({
  ...
  .superRefine((value, context) => {
    if (value.status === "supported" && value.evidenceCount < value.minimumEvidenceCount)
      context.addIssue({
        code: "custom",
        message: "Supported hypotheses require enough evidence.",
        path: ["evidenceCount"],
      });
    if (value.canInfluenceDecision && value.status !== "supported")
      context.addIssue({
        code: "custom",
        message: "Only supported hypotheses may influence decisions.",
        path: ["canInfluenceDecision"],
      });
  });

export type AthleteHypothesis = z.infer<typeof AthleteHypothesisSchema>;

export const AthleteDiscoverySchema = z
  .object({
