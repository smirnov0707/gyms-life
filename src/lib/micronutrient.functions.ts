...
ATHLETE: ${snap.profile.gender}, ${age ?? "?"} y, ${snap.profile.weight === null ? "weight not recorded" : `${snap.profile.weight} kg (${{ measured: "weighed on a scale", photo_estimate: "ESTIMATED BY A MODEL FROM A PHOTOGRAPH, not weighed — do not treat it as exact and say so where it matters", stated: "stated at sign-up, not weighed since" }[snap.profile.weightSource ?? "stated"]})`}, ${snap.profile.height === null ? "height not recorded" : `${snap.profile.height} cm`}, goal ${snap.profile.goal ?? "not recorded"}, diet ${snap.profile.diet}.
