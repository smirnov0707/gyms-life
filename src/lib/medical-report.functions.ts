...
import { z } from "zod";
import { calculateReportAdherence, type AdherenceResult } from "./report-adherence.engine";
...
const ReportSchema = z.object({
  ...
  summary: z.string(),
  /**
   * The model's comment on adherence. The score itself is measured, not
   * asked for: see `calculateReportAdherence`.
   */
  adherenceNote: z.string().default(""),
...
export type MedicalReport = z.infer<typeof ReportSchema> & {
  /** Measured by `calculateReportAdherence`; `note` is the model's comment. */
  adherence: AdherenceResult & { note: string };
  stats: ReportStats;
  generatedAt: string;
};
...
{"headline":"string","summary":"string","adherenceNote":"string","sections":[{"title":"string","body":"string","metrics":[{"label":"string","value":"string","note":"string"}]}],"risks":[{"title":"string","detail":"string","severity":"low"}],"recommendations":[{"title":"string","detail":"string"}],"questionsForDoctor":["string"],"dataGaps":["string"]}`;
    ...
    // Measured from the same stats the model was shown, rather than asked of
    // it. A physician reads this badge; the arithmetic behind it now travels
    // with it.
    const adherence = calculateReportAdherence({
      sessionsPerWeek: stats.sessionsPerWeek,
      plannedSessionsPerWeek: stats.profile?.days_per_week ?? null,
      checkins: stats.checkins,
      nutritionDaysLogged: stats.nutritionDaysLogged,
    });

    return {
      ...parsed,
      adherence: { ...adherence, note: parsed.adherenceNote },
