  ...
  z
    ...
    .superRefine((value, context) => {
      if (value.low > value.high)
        context.addIssue({
          code: "custom",
          message: "Prediction range low must not exceed high",
          path: ["low"],
        });
    }),
...
export const AthletePredictionSchema = z
  .object({
    id: z.string().uuid(),
  ...
  .superRefine((value, context) => {
    if (Date.parse(value.horizonEndsAt) <= Date.parse(value.generatedAt))
      context.addIssue({
        code: "custom",
        message: "Prediction horizon must end after generation",
        path: ["horizonEndsAt"],
      });
    if (value.evaluatedAt && !value.actual)
      context.addIssue({
        code: "custom",
        message: "An evaluated prediction requires an actual outcome",
        path: ["actual"],
      });
    if (value.actual && !value.evaluatedAt)
      context.addIssue({
        code: "custom",
        message: "An actual outcome requires evaluatedAt",
        path: ["evaluatedAt"],
      });
    if (value.target === "workout_completion" && value.predicted.kind !== "probability")
      context.addIssue({
        code: "custom",
        message: "Workout completion predictions must be probabilities",
        path: ["predicted"],
      });
    if (value.target === "workout_completion" && value.actual && value.actual.kind !== "boolean")
      context.addIssue({
        code: "custom",
        message: "Workout completion actual outcomes must be boolean",
        path: ["actual"],
      });
  });
...
export const SimulationScenarioSchema = z
  ...
  .superRefine((value, context) => {
    if (Date.parse(value.horizonEndsAt) <= Date.parse(value.generatedAt))
      context.addIssue({
        code: "custom",
        message: "Simulation horizon must end after generation",
        path: ["horizonEndsAt"],
      });
  });
