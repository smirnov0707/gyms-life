...
export const finishWorkout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: session, error: sessionError } = await supabase
      ...
      .select("id, user_id, plan_id, day_index, started_at, finished_at, duration_seconds, total_volume")
    ...
    if (completed.size !== expected || missing.length > 0) {
      throw new Error(`Cannot finish workout: ${missing.length} planned set(s) are still incomplete.`);
    }
    ...
    const { data: updated, error: updateError } = await supabase
      .from("workout_sessions")
      .update({ finished_at: finishedAt.toISOString(), duration_seconds: durationSeconds, total_volume: Math.round(totalVolume * 100) / 100 })
      .eq("id", session.id)
      .eq("user_id", userId)
      .is("finished_at", null)
