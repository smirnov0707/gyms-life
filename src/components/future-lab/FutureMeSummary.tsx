...
import { buildTwinMetricTrend, type TwinTrendHistory, type TwinTrendMetricKey } from "@/lib/twin-trend";
...
const metrics: Metric[] = [
  ...
  { key: "totalVolumeLast28Days", labelLt: "28 d. krūvis", labelEn: "28d load", unit: "kg", icon: Dumbbell },
...
function scopedHistory(history: TwinTrendHistory, days: number): TwinTrendHistory {
  ...
  return { ...history, points: history.points.filter((point) => Date.parse(point.computedAt) >= cutoff) };
}
...
export function FutureMeSummary() {
  ...
  const history = useMemo(() => (query.data ? scopedHistory(query.data, range) : null), [query.data, range]);
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-[#182846] bg-[#040913] p-5 shadow-[0_35px_100px_rgba(0,0,0,.35)] sm:p-7">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_75%_0%,rgba(109,40,217,.18),transparent_35%),radial-gradient(circle_at_15%_75%,rgba(6,182,212,.08),transparent_30%)]" />
      <div className="relative">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-violet-300">FUTURE ME</p>
          ...
          <div className="flex flex-wrap gap-2" aria-label={isEnglish ? "Observation window" : "Stebėjimo langas"}>
            {ranges.map((days) => (
              <button
                key={days}
                type="button"
        ...
        {query.isError ? (
          <div className="mt-6"><FutureLabEmpty>{isEnglish ? "Trajectory data is temporarily unavailable." : "Trajektorijos duomenys laikinai nepasiekiami."}</FutureLabEmpty></div>
        ) : (
          <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {metrics.map((metric) => {
              ...
              return (
                <FutureLabPanel key={metric.key} title={isEnglish ? metric.labelEn : metric.labelLt} action={<Icon className="size-4 text-cyan-300" />}>
                  {series?.latestValue != null ? (
                    <>
                      <p className="text-3xl font-semibold text-white">
                        {Math.round(series.latestValue * 10) / 10}{metric.unit ? <span className="ml-1 text-sm text-slate-500">{metric.unit}</span> : null}
                      </p>
                      <p className="mt-2 text-[11px] text-slate-500">
                        {series.availability === "available" && series.netChange != null
                          ...
                          : isEnglish ? "Not enough temporal coverage for a direction." : "Krypčiai dar nepakanka laiko aprėpties."}
                      </p>
                    </>
                  ) : (
                    <FutureLabEmpty>{isEnglish ? "No compatible observations in this window." : "Šiame lange nėra suderinamų stebėjimų."}</FutureLabEmpty>
                  )}
                </FutureLabPanel>
              );
            })}
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-[1.3fr_.7fr]">
          <FutureLabPanel eyebrow={isEnglish ? "SIMULATION STATUS" : "SIMULIACIJOS BŪSENA"} title={isEnglish ? "Future simulation remains gated" : "Ateities simuliacija dar užrakinta"} action={<Sparkles className="size-4 text-violet-300" />}>
          ...
          <FutureLabPanel eyebrow={isEnglish ? "MODEL PRINCIPLE" : "MODELIO PRINCIPAS"} title={isEnglish ? "Known ≠ inferred ≠ predicted" : "Known ≠ inferred ≠ predicted"} action={<BrainCircuit className="size-4 text-emerald-300" />}>
            <p className="text-xs leading-relaxed text-slate-400">
              {isEnglish ? "Missing data stays missing. A trend needs at least four compatible observations across 72 hours." : "Trūkstami duomenys lieka trūkstami. Trendui reikia bent keturių suderinamų stebėjimų per 72 valandas."}
            </p>
          </FutureLabPanel>
        </div>
      </div>
