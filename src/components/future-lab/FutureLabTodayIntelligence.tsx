...
export function FutureLabTodayIntelligence() {
  ...
  const notEnough = isEnglish ? "Not enough verified data yet." : "Dar nepakanka patikrintų duomenų.";
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      >
        ...
        <Link to="/progress" className="mt-4 inline-flex items-center gap-1 text-[11px] font-bold text-violet-300 hover:text-violet-200">
          {isEnglish ? "VIEW EVOLUTION" : "ŽIŪRĖTI EVOLIUCIJĄ"} <ArrowUpRight className="size-3" />
        </Link>
      </FutureLabPanel>

      <FutureLabPanel
        eyebrow={isEnglish ? "FUTURE ME" : "FUTURE ME"}
      ...
      >
        {readinessTrend?.latestValue != null ? (
          <div>
            <p className="text-2xl font-semibold text-white">{Math.round(readinessTrend.latestValue)}</p>
            <p className="mt-1 text-xs text-slate-400">
        ...
        <Link to="/progress" className="mt-4 inline-flex items-center gap-1 text-[11px] font-bold text-violet-300 hover:text-violet-200">
          {isEnglish ? "OPEN FUTURE ME" : "ATIDARYTI FUTURE ME"} <ArrowUpRight className="size-3" />
        </Link>
      </FutureLabPanel>
      ...
      >
        {labQuery.isError ? (
          <FutureLabEmpty>{isEnglish ? "Lab data is temporarily unavailable." : "Laboratorijos duomenys laikinai nepasiekiami."}</FutureLabEmpty>
        ) : monitoring ? (
          <div>
            <p className="text-sm leading-relaxed text-slate-200">
              <Statement statementKey={monitoring.statementKey} fallback={isEnglish ? "A new pattern is being monitored." : "Stebimas naujas dėsningumas."} />
            </p>
            ...
            <p className="mt-2 text-[11px] text-slate-500">
              {monitoring.evidenceCount}/{monitoring.minimumEvidenceCount} {isEnglish ? "evidence points" : "įrodymų taškų"}
            </p>
          </div>
        ) : (
          <FutureLabEmpty>{isEnglish ? "No active hypothesis needs testing right now." : "Šiuo metu nėra aktyvios hipotezės, kurią reikėtų tikrinti."}</FutureLabEmpty>
        )}
        <Link to="/lab" className="mt-4 inline-flex items-center gap-1 text-[11px] font-bold text-violet-300 hover:text-violet-200">
      ...
      <FutureLabPanel
        ...
        title={discovery ? (isEnglish ? "Supported by your data" : "Patvirtinta tavo duomenimis") : isEnglish ? "Learning in progress" : "Mokymasis vyksta"}
        action={<BrainCircuit className="size-4 text-emerald-300" />}
      >
        {discovery ? (
          <div>
            <p className="text-sm leading-relaxed text-slate-200">
              <Statement statementKey={discovery.statementKey} fallback={isEnglish ? "A personal pattern has reached the evidence threshold." : "Asmeninis dėsningumas pasiekė reikiamą įrodymų ribą."} />
            </p>
            <p className="mt-3 text-[11px] text-emerald-300">
              {discovery.evidenceCount} {isEnglish ? "evidence points · deterministic" : "įrodymų taškai · deterministinis"}
            </p>
          </div>
        ) : (
          <FutureLabEmpty>{notEnough}</FutureLabEmpty>
        ...
        {calibration ? (
          <p className="mt-3 border-t border-white/[0.05] pt-3 text-[10px] text-slate-500">
            {isEnglish ? "Prediction shadow ledger" : "Prognozių shadow registras"}: {calibration.totalEvaluated}/{calibration.minimumEvaluated} {isEnglish ? "evaluated" : "įvertinta"}
          </p>
        ) : null}
        <Link to="/history" className="mt-3 inline-flex items-center gap-1 text-[11px] font-bold text-violet-300 hover:text-violet-200">
          {isEnglish ? "OPEN JOURNAL" : "ATIDARYTI JOURNAL"} <ArrowUpRight className="size-3" />
        </Link>
      </FutureLabPanel>
    </div>
  );
