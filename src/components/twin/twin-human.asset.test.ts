...
const state: TwinSceneState = {
  ...
  regions: TWIN_BODY_REGIONS.map((id) => ({
    ...
    recoveryPct: 60,
    emphasis: 0.4,
    display: { value: 60, tone: "moderate" },
  })),
};
describe("textured human presentation", () => {
  ...
  it("isolates materials by region and preserves neutral occluders", () => {
    ...
    const chest = new Mesh(new BoxGeometry(), material);
    chest.userData["twinRegion"] = "chest";
    const head = new Mesh(new BoxGeometry(), material);
    head.userData["twinRegion"] = null;
    group.add(chest, head);
    const model = adoptHumanTwinBody(group);
    model.applyAppearance(state, "chest", "evidence");
    expect(chest.material).not.toBe(head.material);
    expect(head.material.color.getHexString()).toBe("ffffff");
    expect(model.meshes).toContain(head);
    expect(model.regionOf.has(head)).toBe(false);
    const original = JSON.stringify(state);
    model.applyAppearance(state, null, "natural");
    expect(chest.material.color.getHexString()).toBe("ffffff");
