...
function tinyGlb(overrides: Record<string, unknown> = {}) {
  ...
  new Uint8Array(buffer, 20, n).fill(32);
  new Uint8Array(buffer, 20, json.length).set(json);
  view.setUint32(20 + n, 4, true);
  view.setUint32(24 + n, 0x004e4942, true);
  return buffer;
}
...
describe("reviewed human asset admission", () => {
  it("admits the actual versioned, embedded candidate and pins its content", () => {
    ...
    expect(
      readFileSync("public/assets/humans/rocketbox-adult-01-v1/LICENSE.txt", "utf8"),
    ).toContain("Copyright (c) 2020 Microsoft");
  });
  it.each([
    { url: "https://third-party.invalid/person.glb" },
    { url: "//third-party.invalid/person.glb" },
    { url: "/assets/humans/../person.glb" },
    { url: "/assets/humans/person.glb?user=private" },
    { byteLength: HUMAN_ASSET_LIMITS.bytes + 1 },
    { sha256: "not-a-hash" },
    { license: "" },
  ...
  it("reads real chunks and rejects 404 rather than parsing it as a model", async () => {
    ...
    await expect(
      readHumanAssetResponse(new Response("missing", { status: 404 })),
    ).rejects.toThrow();
  });
  it("enforces actual transfer size even if Content-Length lies", async () => {
    await expect(
      readHumanAssetResponse(
        new Response(new Uint8Array(HUMAN_ASSET_LIMITS.bytes + 1), {
