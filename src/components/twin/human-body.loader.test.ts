...
describe("human loader ownership and failure paths", () => {
  it("does not parse a 404 or a hash mismatch", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response(new Uint8Array(bytes.byteLength))),
    );
    await expect(loadHumanBody(candidate, new AbortController().signal)).rejects.toThrow(
      "integrity",
    );
  ...
  it("disposes a resolved scene if the request was cancelled during parsing", async () => {
    const own = ownedScene();
    const release = vi.spyOn(own.texture, "dispose");
    const controller = new AbortController();
    parseAsync.mockImplementation(async () => {
      controller.abort();
      return { scene: own.root };
    });
  ...
  it("releases owned resources once and never a separate scene", async () => {
    const own = ownedScene();
    const other = ownedScene();
    const geometry = vi.spyOn(own.geometry, "dispose");
    const material = vi.spyOn(own.material, "dispose");
    const texture = vi.spyOn(own.texture, "dispose");
    const unrelated = vi.spyOn(other.texture, "dispose");
    parseAsync.mockResolvedValue({ scene: own.root });
