...
type Exercise = { name: string; slug?: string; sets?: number; reps?: number | string; rest?: number; rest_seconds?: number; notes?: string }
...
function ActiveWorkoutPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error } = await supabase.from('plans').select('id,title,weeks,days_per_week,data').eq('user_id', user.id).eq('is_active', true).order('created_at', { ascending: false }).limit(1).maybeSingle()
      if (error || !data || !mounted) { if (error) setMessage(error.message); return }
      setPlanId(data.id)
      const raw = (data.data ?? {}) as { days?: Array<{ day?: number; title?: string; exercises?: Exercise[] }> }
      const days = Array.isArray(raw.days) ? raw.days : []
      const weekday = new Date().getDay() === 0 ? 7 : new Date().getDay()
      const selectedIndex = Math.max(0, days.findIndex((d) => Number(d.day) === weekday))
      const today = days[selectedIndex] ?? days[0]
      const list = Array.isArray(today?.exercises) ? today.exercises : []
      if (!list.length) { setMessage('No exercises found in today’s AI program. Rebuild your program if needed.'); return }
      setDayIndex(Number(today?.day ?? selectedIndex + 1))
  ...
  useEffect(() => { if (!session) return; const id = window.setInterval(() => setSeconds(Math.floor((Date.now() - new Date(session.started_at).getTime()) / 1000)), 1000); return () => window.clearInterval(id) }, [session])
  useEffect(() => { if (rest <= 0) return; const id = window.setInterval(() => setRest(v => Math.max(0, v - 1)), 1000); return () => window.clearInterval(id) }, [rest])
  ...
  async function logSet() {
    ...
    setRest(restSeconds); setReps(''); setRpe('')
