...
try {
  ...
  await canvas.evaluate((el) => {
    el.dataset["validationIdentity"] = "original";
  });
  ...
  expect(angles.some((x) => x > 2)).toBe(true);
  expect(angles.some((x) => x < -2)).toBe(true);
  ...
  for (const width of [320, 375, 390, 430]) {
    ...
    await load(p, width === 320 ? "?lang=lt" : "");
    await p.locator("canvas").scrollIntoViewIfNeeded();
  ...
  for (const response of [
    ...
    await expect(p.getByRole("button", { name: "Try 3D again", exact: true })).toBeVisible({
      timeout: 20000,
    });
  ...
  await load(rp);
  await rp.locator("canvas").scrollIntoViewIfNeeded();
  await rp.waitForTimeout(600);
  const frames = await rp.locator("canvas").getAttribute("data-twin-frames");
  await rp.waitForTimeout(650);
  expect(await rp.locator("canvas").getAttribute("data-twin-frames")).toBe(frames);
  await rp.locator("canvas").focus();
  await rp.keyboard.press("ArrowRight");
  await rp.waitForTimeout(150);
  expect(Number(await rp.locator("canvas").getAttribute("data-twin-frames"))).toBeGreaterThan(
    Number(frames),
  );
  await reduced.close();
  record("reduced motion stops idle rendering but preserves user rotation");
  const video = await browser.newContext({
    viewport: { width: 390, height: 844 },
    recordVideo: { dir: path.join(artifacts, "video"), size: { width: 390, height: 844 } },
  });
  const vp = await video.newPage();
  await load(vp);
  await vp.locator("canvas").scrollIntoViewIfNeeded();
  await vp.locator("canvas").focus();
  for (let i = 0; i < 16; i++) {
  ...
  const metrics = await page.locator("canvas").evaluate((el) => ({
    drawCalls: Number(el.dataset.twinDrawCalls),
    triangles: Number(el.dataset.twinTriangles),
    deviceFps: null,
    environment: "GitHub Actions Chromium SwiftShader, not physical-phone validation",
  }));
  expect(errors).toEqual([]);
  record("browser diagnostics captured; visual acceptance left to owner");
  await writeFile(
    path.join(artifacts, "results.json"),
    JSON.stringify({ results, angles, metrics, errors, visualAccepted: false }, null, 2),
  );
} catch (error) {
  console.error(error);
  process.exitCode = 1;
  ...
  await writeFile(
    ...
    JSON.stringify({ results, error: String(error), errors }, null, 2),
  );
} finally {
  await browser?.close();
  await server?.close();
