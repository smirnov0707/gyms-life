...
const candidate = JSON.parse(await readFile(path.join(root, "src/components/twin/human-asset.candidate.json"), "utf8"));
const results = [];
...
const controls = async (p, open, lt = false) => {
  const toggle = p.getByRole("button", { name: lt ? "Vaizdo valdymas" : "View controls", exact: true });
...
const preset = async (p, name) => {
  ...
  await controls(p, false); await p.locator("canvas").scrollIntoViewIfNeeded(); await p.waitForTimeout(200);
};
...
const stop = async (p) => { await controls(p, true); await p.getByLabel("Ambient motion", { exact: true }).uncheck(); await controls(p, false); };
try {
  ...
  const context = await browser.newContext({ viewport: { width: 1280, height: 1100 }, reducedMotion: "reduce", recordVideo: { dir: path.join(artifacts, "video"), size: { width: 1280, height: 1100 } } });
  page = await context.newPage(); page.on("pageerror", e => errors.push(String(e)));
  await load(page); await stop(page);
  ...
  await writeFile(path.join(artifacts, "render-metrics.json"), JSON.stringify({
    environment: "GitHub runner Chromium SwiftShader; NOT physical phone FPS/battery", asset: candidate,
  ...
  for (const [button, file] of [["Front","front"],["Back","back"],["Left side","left"],["Right side","right"]]) {
  ...
  await preset(page, "Front"); await preset(page, "Rotate right"); await preset(page, "Rotate right");
  await canvas.screenshot({ path: path.join(artifacts, "body-three-quarter.png") });
  const visited = [];
  for (let i = 0; i < 16; i++) { await preset(page, "Rotate right"); visited.push(Number(await canvas.getAttribute("data-twin-yaw"))); }
  expect(visited.some(n => n > 1)).toBe(true); expect(visited.some(n => n < -1)).toBe(true);
  ...
  const yaw = await canvas.getAttribute("data-twin-yaw"); const distance = await canvas.getAttribute("data-twin-distance");
  ...
  expect(await page.evaluate(() => window.__reviewCanvas === document.querySelector("canvas"))).toBe(true);
  ...
  record("natural appearance does not change evidence; layer, camera, canvas and selection persist");
  ...
  const geometryReview = await page.evaluate(async ({ moduleUrl, descriptor }) => {
  ...
  await writeFile(path.join(artifacts, "region-review.json"), JSON.stringify(geometryReview, null, 2));
  ...
  expect(geometryReview.groups.sort()).toEqual(["abs","arms","back","chest","core","glutes","legs","shoulders"]);
  record("actual nearest-surface region selection cannot select chest through back or neutral head");

  await page.getByRole("button", { name: "2D", exact: true }).click();
  await expect(page.locator('[data-twin-stage="2d"]')).toBeVisible();
  await expect(page.getByRole("heading", { name: "Chest", exact: true })).toBeVisible();
  await page.getByRole("button", { name: "3D", exact: true }).click();
  await expect(page.locator('canvas[data-twin-appearance="human"]')).toBeVisible({ timeout: 30000 });
  await page.locator("canvas").evaluate(node => (node.getContext("webgl2") || node.getContext("webgl")).getExtension("WEBGL_lose_context").loseContext());
  await expect(page.getByText("3D is unavailable on this device. Your evidence is still available in 2D.", { exact: true })).toBeVisible();
  ...
  await page.locator("[data-twin-viewport]").evaluate(node => { node.style.height = "1800px"; });
  await preset(page, "Front");
  await page.locator("canvas").screenshot({ path: path.join(artifacts, "human-detail-front.png") });
  await context.close();

  for (const width of [320,375,390,430]) {
    const mobile = await browser.newContext({ viewport: { width, height: 900 }, isMobile: true, hasTouch: true, deviceScaleFactor: 1, reducedMotion: "reduce" });
    page = await mobile.newPage(); const lt = width === 320;
    await load(page, lt ? "lt" : "en");
    const overflow = await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 1);
    expect(overflow).toBe(false);
    const taps = await page.locator("[data-human-preview] button").evaluateAll(nodes => nodes.every(n => n.getBoundingClientRect().height >= 44 && n.getBoundingClientRect().width >= 44));
    expect(taps).toBe(true);
    await page.screenshot({ path: path.join(artifacts, `mobile-${width}${lt ? "-lt" : ""}.png`), fullPage: true });
    if (width === 390) {
      await page.getByRole("button", { name: "Light theme", exact: true }).click();
      await page.screenshot({ path: path.join(artifacts, "human-light-theme.png"), fullPage: true });
  ...
  await page.route("**/assets/humans/**/human.glb", route => route.fulfill({ status: 404, body: "missing" }));
  await page.goto("http://127.0.0.1:4182");
  await expect(page.getByRole("button", { name: "Retry human model", exact: true })).toBeVisible({ timeout: 30000 });
  ...
  if (page && !page.isClosed()) {
    ...
    const diagnostic = await page.evaluate(async ({ moduleUrl, descriptor }) => {
      try {
        const { inspectHumanCandidate } = await import(moduleUrl);
        return await inspectHumanCandidate(descriptor);
      } catch (error) { return { loaderError: String(error) }; }
    }, { moduleUrl: "/@fs" + path.join(root, "tests/human-browser/probe.ts"), descriptor: candidate }).catch(error => ({ error: String(error) }));
    await writeFile(path.join(artifacts, "loader-diagnostic.json"), JSON.stringify(diagnostic, null, 2));
    await writeFile(path.join(artifacts, "page-diagnostic.txt"), await page.locator("body").innerText().catch(() => "unavailable"));
  }
  console.error(error); process.exitCode = 1;
} finally {
  await writeFile(path.join(artifacts, "results.json"), JSON.stringify({ results, errors, synthetic: true, physicalDeviceTest: false, visualAcceptance: "pending" }, null, 2));
