...
const server = await createServer({
  ...
  plugins: [
    { name: "test-only-no-backend", resolveId(id) { if (id === "@/lib/digital-twin.functions") return "\0twin-test-service"; }, load(id) { if (id === "\0twin-test-service") return 'export function getTwinSnapshot() { throw new Error("Test fixture cannot access backend") }'; } },
    react(), tailwindcss(),
  ],
...
const loaded = async (target) => {
  ...
  await expect.poll(async () => Number(await target.locator("canvas").getAttribute("data-twin-frames"))).toBeGreaterThan(0);
};
...
try {
  ...
  await page.mouse.move(x + 95, y + 6, { steps: 12 }); await page.mouse.up();
  await page.waitForTimeout(250);
  ...
  await expect.poll(async () => Number(await canvas.getAttribute("data-twin-distance"))).toBeLessThan(beforeZoom);
  ...
  await expect(page.getByText("No sets logged for this region recently.", { exact: true })).toBeVisible();
  ...
  await client.send("Input.dispatchTouchEvent", { type: "touchStart", touchPoints: [{ x: center.x - 30, y: center.y, id: 1 }, { x: center.x + 30, y: center.y, id: 2 }] });
  await client.send("Input.dispatchTouchEvent", { type: "touchMove", touchPoints: [{ x: center.x - 55, y: center.y, id: 1 }, { x: center.x + 55, y: center.y, id: 2 }] });
  await client.send("Input.dispatchTouchEvent", { type: "touchEnd", touchPoints: [] });
  await expect.poll(async () => Number(await mobileCanvas.getAttribute("data-twin-distance"))).toBeLessThan(distanceBeforePinch);
  await page.getByRole("button", { name: "Reset view", exact: true }).click();
  ...
  page = await reduced.newPage(); await loaded(page); await page.waitForTimeout(1000);
  ...
  page = await unsupported.newPage(); await page.goto("http://127.0.0.1:4179");
  await expect(page.getByText("3D is unavailable on this device. Your evidence is still available in 2D.", { exact: true })).toBeVisible({ timeout: 30000 });
