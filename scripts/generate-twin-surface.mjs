...
function ellipsoid(center, radii, blend = 0.025, roll = 0) {
  ...
  primitives.push({
    center, radius: Math.max(...radii) + blend * 2,
    blend,
    distance(x, y, z) {
...
ellipsoid([0, 1.319, -0.003], [0.188, 0.190, 0.110], 0.045);
ellipsoid([0, 1.141, -0.010], [0.139, 0.196, 0.092], 0.050);
ellipsoid([0, 0.988, -0.005], [0.156, 0.137, 0.105], 0.045);
ellipsoid([0, 1.453, -0.006], [0.198, 0.061, 0.085], 0.035);
taperedLimb([0, 1.488, -0.012], [0, 1.627, -0.014], 0.056, 0.049, 0.98, 0.030);
...
ellipsoid([0, 1.677, 0.007], [0.061, 0.061, 0.070], 0.020);
ellipsoid([0, 1.649, 0.025], [0.040, 0.025, 0.040], 0.013);
ellipsoid([0, 1.714, 0.072], [0.010, 0.028, 0.018], 0.010);
for (const side of [-1, 1]) {
  // Pectorals merge into the ribcage instead of floating on it.
  ellipsoid([side * 0.087, 1.359, 0.071], [0.104, 0.068, 0.050], 0.030, -side * 0.07);
  ellipsoid([side * 0.079, 1.389, -0.071], [0.087, 0.093, 0.043], 0.029);
  ellipsoid([side * 0.120, 1.269, -0.040], [0.066, 0.150, 0.061], 0.035, -side * 0.15);
  ...
  taperedLimb([side * 0.231, 1.409, -0.006], [side * 0.311, 1.131, 0.006], 0.054, 0.034, 0.98, 0.028);
  ellipsoid([side * 0.266, 1.287, 0.014], [0.047, 0.118, 0.048], 0.022, side * 0.29);
  taperedLimb([side * 0.311, 1.130, 0.006], [side * 0.359, 0.884, 0.025], 0.033, 0.021, 0.92, 0.024);
  ellipsoid([side * 0.328, 1.051, 0.010], [0.037, 0.088, 0.033], 0.020, side * 0.20);
  // Palm and gently grouped fingers; no implied fine motor measurement.
  ellipsoid([side * 0.367, 0.841, 0.028], [0.030, 0.056, 0.018], 0.020, side * 0.15);
  for (let finger = 0; finger < 4; finger++) {
    const x = side * (0.348 + finger * 0.012);
    taperedLimb([x, 0.815, 0.030], [x + side * 0.009, 0.755 + Math.abs(finger - 1.3) * 0.010, 0.037], 0.0075, 0.006, 0.95, 0.006);
  }
  taperedLimb([side * 0.344, 0.861, 0.031], [side * 0.326, 0.813, 0.047], 0.011, 0.0075, 0.95, 0.010);
  // Posterior pelvis blends into thighs; knees and ankles stay connected.
  ellipsoid([side * 0.078, 0.956, -0.060], [0.087, 0.095, 0.081], 0.029);
  taperedLimb([side * 0.082, 0.944, -0.003], [side * 0.099, 0.547, 0.014], 0.087, 0.047, 1.02, 0.028);
  ellipsoid([side * 0.091, 0.770, 0.022], [0.077, 0.154, 0.074], 0.018, side * 0.035);
  taperedLimb([side * 0.099, 0.547, 0.014], [side * 0.100, 0.143, -0.013], 0.043, 0.026, 1.06, 0.023);
  ellipsoid([side * 0.100, 0.387, -0.025], [0.049, 0.130, 0.051], 0.023);
  ellipsoid([side * 0.100, 0.095, 0.008], [0.037, 0.055, 0.056], 0.020);
  ellipsoid([side * 0.104, 0.065, 0.076], [0.047, 0.032, 0.090], 0.021);
}
...
for (const shape of primitives) {
  const start = shape.center.map((v, i) => clamp(Math.floor((v - shape.radius - ORIGIN[i]) / STEP), 0, SIZE[i] - 1));
  const end = shape.center.map((v, i) => clamp(Math.ceil((v + shape.radius - ORIGIN[i]) / STEP), 0, SIZE[i] - 1));
  for (let z = start[2]; z <= end[2]; z++) {
    for (let y = start[1]; y <= end[1]; y++) {
      for (let x = start[0]; x <= end[0]; x++) {
        const id = node(x, y, z);
        const distance = shape.distance(ORIGIN[0] + x * STEP, ORIGIN[1] + y * STEP, ORIGIN[2] + z * STEP);
...
for (let z=0;z<NZ-1;z++) for (let y=0;y<NY-1;y++) for (let x=0;x<NX-1;x++) {
  ...
  const points=corners.map(([dx,dy,dz])=>[ORIGIN[0]+(x+dx)*STEP,ORIGIN[1]+(y+dy)*STEP,ORIGIN[2]+(z+dz)*STEP]);
  for (const tetra of tetrahedra) {
    const inside=tetra.filter(i=>field[ids[i]]<0), outside=tetra.filter(i=>field[ids[i]]>=0);
    if (!inside.length||!outside.length) continue;
    const outward=[0,1,2].map(axis=>outside.reduce((sum,i)=>sum+points[i][axis],0)/outside.length-inside.reduce((sum,i)=>sum+points[i][axis],0)/inside.length);
    ...
    if (inside.length===1) triangle(...outside.map(i=>cut(inside[0],i)),outward);
      ...
      const ac=cut(a,c),ad=cut(a,d),bc=cut(b,c),bd=cut(b,d);
...
for(const strength of [0.45,-0.47,0.45,-0.47]){
  ...
  for(let i=0;i<neighbours.length;i++) for(let axis=0;axis<3;axis++){
    let avg=0;for(const other of neighbours[i]) avg+=positions[other*3+axis];
...
const [simplified] = MeshoptSimplifier.simplify(new Uint32Array(indices), new Float32Array(positions), 3, 48000, 0.001);
...
function regionAt(x,y,z){
  ...
  if(a>0.180 && y>1.354) return "shoulders";
  const armBoundary=y>1.12?0.202+(1.35-y)*0.20:0.255;
  ...
  if(y<=1.278 && z>0.064 && a<0.080) return "abs";
...
for(let i=0;i<indices.length;i+=3){
  ...
  const point=[0,1,2].map(axis=>face.reduce((sum,id)=>sum+positions[id*3+axis],0)/3);
  groups[regionAt(...point)].push(...face);
}
const output={version:"generic-human-surface-1",scale:SCALE,positions:positions.map(v=>Math.round(v*SCALE)),groups};
const text=JSON.stringify(output)+"\n";
const gzipBytes=gzipSync(text).length;
if (indices.length / 3 > 24000 || positions.length / 3 > 15000 || gzipBytes > 210000) throw new Error("Twin surface exceeds the declared asset budget");
const out=resolve(process.env.TWIN_SURFACE_OUTPUT ?? "src/components/twin/twin-body.surface.json");
mkdirSync(dirname(out),{recursive:true});writeFileSync(out,text);
console.log(JSON.stringify({vertices:positions.length/3,triangles:indices.length/3,gzipBytes,groups:Object.fromEntries(Object.entries(groups).map(([name,faces])=>[name,faces.length/3]))},null,2));
